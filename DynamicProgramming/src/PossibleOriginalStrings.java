...
public class PossibleOriginalStrings {
    public int possibleStringCount(String word, int k) {
        int MOD = 1_000_000_007;
        List<Integer> groups = getConsecutiveLetters(word);
        int totalCombinations = (int) groups.stream().mapToLong(Integer::longValue).reduce(1L, (a, b) -> a * b % MOD);
        if (k <= groups.size()) {
        ...
        int invalidCombinations = Arrays.stream(dp).reduce(0, (a, b) -> (a + b) % MOD);
